...
router.get("/", async(req,res)=> {
    try {
        ...
        if (department) {
            const deptPattern = `%${String(department).replace(/[%_]/g, '\\$&')}%`;//Help to prevent SQL injections
            filterConditions.push(ilike(departments.name, deptPattern));//Help to prevent SQL injections
        }
